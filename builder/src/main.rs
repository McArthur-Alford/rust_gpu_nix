...
fn build_shader(path_to_crate: &str, env_var: &str) -> Result<(), Box<dyn Error>> {
    ...
    if let Ok(out_dir) = std::env::var("OUT_DIR") {
        println!("cargo:rustc-env={}_PATH={}",env_var,result.module.unwrap_single().display());
        ...
        println!("cargo:rustc-env={}_ENTRYPOINTS={}",env_var,dest_path.display());
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    build_shader("../crates/simplest-shader", "SIMPLEST_SHADER")?;
    Ok(())
}
